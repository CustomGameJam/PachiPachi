...
public class Grid : MonoBehaviour
{
    ...
    public IEnumerator ChangeAnimation(int level,string player)
    {
        ...
        currentBomb = Instantiate(getPlayerBombs(player)[level-1], transform.position + new Vector3(0, 1, 0), transform.rotation);
    ...
    public IEnumerator BoomAnimation(string player,int[] locations)
    {
        ...
        for (var j = 0; j < createdObjects.Count; j++)
        {
            createdObjects[j].transform.DOJump(_board.FindGridPosition(locations[j])+ Vector3.up, 1f, 1, .5f);
            createdObjects[j].transform.DOScale(new Vector3(.3f, .3f, .3f), .4f);
        }
